...
export async function initializePostgres() {
  ...
  try {
    ...
    if (Number(userCheck.rows[0].count) === 0) {
      ...
      const operatorHash = crypto.createHash('sha256').update('operador').digest('hex');
      const auditorHash = crypto.createHash('sha256').update('auditor').digest('hex');
...
export const pgOperations = {
  getAll: async (search?: string, category?: string, lowStock?: boolean): Promise<InventoryItem[]> => {
  ...
  getBySku: async (sku: string): Promise<InventoryItem | null> => {
    const res = await pool.query(`
  ...
  create: async (item: Omit<InventoryItem, 'id' | 'lastUpdated'> & { permitir_backorder?: boolean }): Promise<InventoryItem> => {
    const res = await pool.query(`
  ...
  update: async (id: string, item: Partial<Omit<InventoryItem, 'id' | 'lastUpdated'> & { permitir_backorder?: boolean }>): Promise<InventoryItem> => {
    const fields: string[] = [];
    const params: any[] = [id];
    ...
    if (fields.length === 0) {
      const existing = await pgOperations.getById(id);
  ...
  restore: async (id: string): Promise<any> => {
    ...
    if (res.rows.length === 0) throw new Error('Item not found in recycle bin');
    const row = res.rows[0];
  ...
  getMovements: async (id_produto?: string): Promise<any[]> => {
    let query = `
  ...
  generateDailyTasks: async (): Promise<any[]> => {
    ...
    // Return all tasks
    return pgOperations.getTasks();
  },

  completeTask: async (id_tarefa: string, data: { quantidade_conferida: number; observacao?: string }): Promise<any> => {
    const client = await pool.connect();
    try {
      await client.query('BEGIN');
      ...
      if (taskRes.rows.length === 0) throw new Error('Tarefa de conferência não encontrada.');
      const task = taskRes.rows[0];
      if (task.status === 'CONCLUIDO') throw new Error('Esta tarefa já foi concluída.');
  ...
  createTask: async (data: { id_produto: string; tipo_tarefa: string; quantidade_esperada: number }): Promise<any> => {
    const res = await pool.query(`
      INSERT INTO tarefas_conferencia (id_produto, tipo_tarefa, quantidade_esperada, status)
  ...
  createUser: async (data: { username: string; password_cleartext: string; nome: string; perfil: string }): Promise<any> => {
    const password_hash = crypto.createHash('sha256').update(data.password_cleartext).digest('hex');
    const res = await pool.query(`
  ...
  authenticateUser: async (username: string, password_cleartext: string): Promise<any | null> => {
    const password_hash = crypto.createHash('sha256').update(password_cleartext).digest('hex');
    const res = await pool.query(`
  ...
  importDatabase: async (data: Record<string, any[]>): Promise<void> => {
    const client = await pool.connect();
    try {
      await client.query('BEGIN');
