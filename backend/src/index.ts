...
import { pgOperations, initializePostgres } from './postgresDb';
...
async function pruneOldBackups(): Promise<number> {
  ...
  for (const file of files) {
    if (!file.startsWith('wms-backup-') || !file.endsWith('.json')) continue;
...
async function createBackupFile(): Promise<string> {
  const data = await pgOperations.exportDatabase();
  const now = new Date();
...
app.get('/api/inventory', async (req, res) => {
  ...
  try {
    const items = await pgOperations.getAll(
      search as string,
      category as string,
      lowStock === 'true'
    );
...
app.get('/api/inventory/stats', async (req, res) => {
  try {
    const stats = await pgOperations.getStats();
    res.json(stats);
  } catch (error: any) {
    res.status(500).json({ error: error.message || 'Database error' });
  }
...
app.get('/api/inventory/movements', async (req, res) => {
  ...
  try {
    const logs = await pgOperations.getMovements(id_produto as string);
    res.json(logs);
  } catch (error: any) {
    res.status(500).json({ error: error.message || 'Database error' });
  }
...
app.post('/api/inventory/movements', async (req, res) => {
  ...
  try {
    const result = await pgOperations.recordMovement({
      id_produto,
      tipo_movimentacao,
...
app.get('/api/inventory/tasks', async (req, res) => {
  try {
    const tasks = await pgOperations.generateDailyTasks();
    res.json(tasks);
  } catch (error: any) {
    res.status(500).json({ error: error.message || 'Database error' });
  }
...
app.post('/api/inventory/tasks', async (req, res) => {
  ...
  try {
    const task = await pgOperations.createTask({
      id_produto,
      tipo_tarefa,
...
app.post('/api/inventory/tasks/:id/complete', async (req, res) => {
  ...
  try {
    const result = await pgOperations.completeTask(req.params.id, {
...
app.get('/api/inventory/ai-suggestions', async (req, res) => {
  try {
    ...
    const activeGenAI = customKey ? new GoogleGenerativeAI(customKey) : genAI;
    ...
    const mockActionItems = lowStockProducts.map(p => 
      `Comprar aproximadamente ${p.minStockLevel * 2 - p.quantity} unidades de "${p.name}" (SKU: ${p.sku}) para reestabelecer margem segura.`
    );
...
app.get('/api/inventory/:id', async (req, res) => {
  try {
    const item = await pgOperations.getById(req.params.id);
    if (!item) {
...
app.post('/api/inventory', async (req, res) => {
  ...
  try {
    const existing = await pgOperations.getBySku(sku);
    if (existing) {
      res.status(400).json({ error: 'SKU already exists' });
      return;
    }

    const newItem = await pgOperations.create({
      name,
      sku,
      category,
...
app.put('/api/inventory/:id', async (req, res) => {
  ...
  try {
    const existing = await pgOperations.getById(req.params.id);
    if (!existing) {
      res.status(404).json({ error: 'Item not found' });
      return;
    }
    ...
    if (sku && sku !== existing.sku) {
      const skuCheck = await pgOperations.getBySku(sku);
      if (skuCheck) {
        res.status(400).json({ error: 'SKU already exists' });
        return;
      }
    ...
    const updatedItem = await pgOperations.update(req.params.id, {
      name,
      sku,
      category,
...
app.get('/api/inventory/trash', async (req, res) => {
  try {
    const trash = await pgOperations.getTrash();
    res.json(trash);
  } catch (error: any) {
    res.status(500).json({ error: error.message || 'Database error' });
  }
...
app.post('/api/inventory/trash/:id/restore', async (req, res) => {
  try {
    const restored = await pgOperations.restore(req.params.id);
...
app.delete('/api/inventory/trash/:id', async (req, res) => {
  try {
    await pgOperations.deletePermanently(req.params.id);
...
app.delete('/api/inventory/:id', async (req, res) => {
  try {
    const deleted = await pgOperations.delete(req.params.id);
...
app.post('/api/auth/login', async (req, res) => {
  ...
  try {
    const user = await pgOperations.authenticateUser(username, password);
    if (!user) {
...
app.get('/api/users', async (req, res) => {
  try {
    const users = await pgOperations.getUsers();
    res.json(users);
  } catch (error: any) {
    res.status(500).json({ error: error.message || 'Database error' });
  }
...
app.post('/api/users', async (req, res) => {
  ...
  try {
    const user = await pgOperations.createUser({
      username,
...
app.delete('/api/users/:id', async (req, res) => {
  try {
    const success = await pgOperations.deleteUser(req.params.id);
    if (!success) {
...
app.post('/api/backups/restore', async (req, res) => {
  ...
  try {
    ...
    await pgOperations.importDatabase(backupData);
