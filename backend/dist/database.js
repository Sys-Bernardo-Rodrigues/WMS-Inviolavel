...
function initializeDatabase() {
  ...
  `, (err) => {
        ...
        db.get('SELECT COUNT(*) as count FROM inventory', [], (countErr, row) => {
            if (countErr) {
          ...
          VALUES (?, ?, ?, ?, ?, ?, ?, ?, ?)
        `);
                mockData_1.initialInventory.forEach(item => {
                    stmt.run([
                        item.id,
                        item.name,
                        item.sku,
...
exports.dbOperations = {
    getAll: (search, category, lowStock) => {
    ...
    getBySku: (sku) => {
        return new Promise((resolve, reject) => {
            db.get('SELECT * FROM inventory WHERE LOWER(sku) = LOWER(?)', [sku], (err, row) => {
                if (err)
                    reject(err);
                else
                    resolve(row || null);
    ...
    create: (item) => {
        return new Promise((resolve, reject) => {
            ...
            const newItem = { id, ...item, lastUpdated };
            db.run(`
        INSERT INTO inventory (id, name, sku, category, quantity, price, minStockLevel, location, lastUpdated)
        VALUES (?, ?, ?, ?, ?, ?, ?, ?, ?)
      `, [
    ...
    update: (id, item) => {
        return new Promise(async (resolve, reject) => {
            try {
                const existing = await exports.dbOperations.getById(id);
                if (!existing) {
    ...
    getStats: () => {
        return new Promise((resolve, reject) => {
            db.all('SELECT category, quantity, price, minStockLevel FROM inventory', [], (err, rows) => {
                if (err) {
                    reject(err);
                    return;
                }
